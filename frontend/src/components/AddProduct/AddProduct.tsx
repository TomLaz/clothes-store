...
import { CategoryI } from '../../models/Global.model';

const AddProduct: React.FC = () => {
    const { id } = useParams<{ id: string }>();
    const { data: { jwt, products, categories,
        email, role, loading, basket }, updateBasket } = useContext( GlobalContext );
    const history = useHistory();

    const product = ( products.length > 0 && !!id ) ?
        products.find( ( prod ) => prod.id.toString() === id.toString() ) : '';
    const category = !!product ? categories.find( ( cat: CategoryI ) => cat.id.toString() === product?.categoryId.toString() ) : '';
    ...
    useEffect( () => {
        ...
        if ( !!product && sizeSelected === '' ) {
            !isUnmounted && onOptionSelected( product.sizes[0].size, product.sizes[0].stock );
        }
    ...
    const addProductHandler = (): void => {
        if ( email === undefined || role === '' ) {
            history.push( GlobalService.states.signIn );
        } else {
            const userBasket = JSON.parse( JSON.stringify( basket ) );
            ...
            updateBasket( userBasket );
            GlobalService.updateBasket( jwt || '', userBasket );

            setProductAdded( true );
            !!product && !!product.sizes && onOptionSelected( product.sizes[0].size, product.sizes[0].stock );
